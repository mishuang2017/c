...
main(int argc, char *argv[])
{
	...
	if (server_flag == B_TRUE) {
		struct timespec         start_timeget_rusage;
		...
		our_get_current_usage(&start_usage);
		our_get_current_time(&start_timeget_rusage);

		server_corrupt(port, lwp, v6);

                our_get_current_usage(&stop_usage);
		our_get_current_time(&stop_time);

                dlapse = our_get_elapsed_time(&start_timeget_rusage,
                    &stop_time, &elapsed_time);

                our_get_elapsed_usage(&start_usage, &stop_usage,
                ...
                fprintf(stderr, "%ld.%09ld seconds elapsed time\n",
                    elapsed_time.tv_sec, elapsed_time.tv_nsec);
                fprintf(stderr, "%ld.%09ld seconds user time (%.4f%%)\n",
                    elapsed_user.tv_sec, elapsed_user.tv_nsec,
                    duser);
                fprintf(stderr, "%ld.%09ld seconds kernel time (%.4f%%)\n",
	...
	if (client_flag == B_TRUE) {
		...
		client_corrupt(port, lwp, v6, run_time, server_name);
