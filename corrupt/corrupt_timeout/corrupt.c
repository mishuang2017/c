...
*ior_timeout(void *arg)
{
	...
	int n = 0;
	while ((int)(current_time - start_time) < run_time) {
		if (total_bytes_saved == total_bytes && total_bytes != 0) {
			printf("data transfer suspended %d seconds\n",
			    ++n);
		} else {
			if (n != 0)
				printf("data transfer resumes after %d "
				    "seconds\n", n);
		...
		if (n == kill_time) {
			printf("data transfer stopped %d seconds\n",
			    n++);
			exit(20);
		}
...
main(int argc, char *argv[])
{
	...
	pthread_t ior_thread;
	extern char *optarg;

	while ((c = getopt(argc, argv, "b:c:dhk:l:n:p:st:v:x:")) != -1) {
		switch (c) {
			case 'b':
				bind_name = optarg;
				break;
	...
	if (client_flag == B_TRUE) {
		...
		if ((pthread_create(&ior_thread, NULL,
		    ior_timeout, (void *)NULL)) != 0) {
			perror("pthread_create() error");
			exit(1);
		}
